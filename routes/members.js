...
router.get("/api/orders", async (req, res) => {
    const sql =
        "SELECT m.`m_sid`,m.`email`,o.`amount`,o.`order_sid`,o.`order_date`,tp.`room_sid`,tp.`start`,tp.`end` FROM `members` m JOIN `orders` o ON m.`m_sid`=o.`users_sid` JOIN `orders_details_live` tp ON o.`order_sid`=tp.`orders_sid` ORDER BY m.`m_sid` DESC";
    const [rs2] = await db.query(sql);
    rs2.forEach((el) => {
        // let str = res.locals.toDateString(el.birthday);
        let str2 = res.locals.toDatetimeString(el.order_date);
...
router.post("/edit/:sid", async (req, res) => {
    ...
    if (res.locals.auth && res.locals.auth.email) {
        ...
        // 取原先資料庫加密過後的密碼
        const selectPass =
            "SELECT password FROM members WHERE m_sid=" + `${m_sid}`;
        ...
        if (password === "") {
            ...
            const [rs] = await db.query(
                `UPDATE members SET m_name=?,gender=?,birthday=?,m_address=?  WHERE m_sid=${m_sid}`,
            ...
            if (bcrypt.compareSync(password, rs1[0].password)) {
                ...
                // return res.json(password)
                const [rs3] = await db.query(
                    `UPDATE members SET m_name=?,gender=?,password=?,birthday=?,m_address=?  WHERE m_sid=${m_sid}`,
                    [name, gender, password, birthday, address]
                );
                // return res.json(rs)
...
router.post("/signup", upload.none(), async (req, res) => {
    ...
    if (!newAr.length) {
        try {
            const sql =
                "INSERT INTO members ( `email`, `m_name`,`gender` ,`birthday`,`password`,`grade_sid`,`check_code`,`check_email`) VALUES (?,?,?,?,?,?,?,?)";
        ...
        if (output.success) {
            ...
            let newCode = jwt.sign(
                { m_sid: output.result.insertId, email: req.body.email },
                process.env.JWT_KEY
            );
...
router.get("/product-like", async (req, res) => {
    const sql =
        "SELECT `ProductSid`,`ProductsName`,`ProductsPrice`,`ProductsMainPic` FROM `products`";
    const [rs] = await db.query(sql);
...
router.get("/product-like-info/:m_id", async (req, res) => {
    ...
    if (!rs.length) {
        ...
        // 資料庫已收藏的商品
        const someCombined = pIdtoString.reduce(function (a, b) {
            return a.concat(b);
        }, []);
...
router.post("/creditcard/add", async (req, res) => {
    ...
    if (res.locals.auth && res.locals.auth.email) {
        ...
        const sql =
            "INSERT INTO `credit_card`(`credit_num`,`credit_name`, `credit_date`, `credit_code`,`m_id`) VALUES (?,?,?,?,?)";
        const [rs] = await db.query(sql, [
            number,
            name,
            expiry,
            cvc,
            res.locals.auth.m_sid,
        ]);
        // return res.json(rs);
        if (!rs.insertId) {
            output.error = "沒有新增";
...
router.get("/creditcard/:sid", async (req, res) => {
    ...
    if (res.locals.auth && res.locals.auth.email) {
        ...
        if (!rs.length) {
            ...
            rs.map((v, i) => {
                list.push({
                    credit_sid: v.credit_sid,
                    credit_num: v.credit_num,
...
router.get("/bonus/list/:sid", async (req, res) => {
    ...
    const sql =
        "SELECT bl.bonusList_sid,bl.getTime_start,bl.getTime_end,bl.bonus_status,bp.name,bp.number,bp.limitDate FROM bonus_list bl JOIN bonus_point bp ON bl.point_id=bp.point_sid WHERE bl.m_id=? ORDER BY bonusList_sid DESC";
...
router.post("/convenience-store", async (req, res) => {
    ...
    const sql2 =
        "SELECT `city`,`area`,`store_name` FROM `convenience_store` WHERE `m_id`=?";
    ...
    if (!ar.length) {
        try {
            ...
            if (!rs.insertId) {
                ...
                const sql2 =
                    "SELECT `store_sid`,`store_name` FROM `convenience_store` WHERE `store_sid`=?";
...
router.get("/convenience-store", async (req, res) => {
    ...
    const sql =
        "SELECT store_sid,store_name FROM convenience_store WHERE m_id=?";
